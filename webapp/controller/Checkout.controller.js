...
sap.ui.define([
	...
	return Controller.extend("com.sap.espm.shop.controller.Checkout", {
		...
		wizardCompletedHandler: function() {
			...
			if (validationFlag === false || firstName.length === 0 || lastName.length === 0 || birthDate.length === 0 || eMail.length === 0 ||
				street.length === 0 || city.length === 0 || postalCode.length === 0 || country.length === 0 ||
				name.length === 0 || cardNumber.length === 0 || secNumber.length === 0 || (!street.match(myInteger)) === true || (!postalCode.match(
					myInteger)) === true || (name.match(myInteger)) === true || (!cardNumber.match(myInteger)) === true ||
				firstName.match(myInteger) === true || lastName.match(myInteger) === true || city.match(myInteger) === true || country.match(
					myInteger) === true) {
		...
		checkCustomerInformation: function() {
			if (this.byId("firstNameId").getValue().length === 0 || this.byId("lastnameId").getValue().length === 0 || this.byId("birthId").getValue()
				.length === 0 || this.byId("newEmailId").getValue().length === 0 ||
				this.byId("streetId").getValue().length === 0 || this.byId("cityId").getValue().length === 0 || this.byId("postalId").getValue().length ===
				0 || this.byId("countryListId").getSelectedKey().length === 0) {
